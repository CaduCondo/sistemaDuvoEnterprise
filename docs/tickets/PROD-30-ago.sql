-- ============================================================================
-- PRODUÇÃO — 30/ago/2026 — RODE ESTE ARQUIVO INTEIRO, DE UMA VEZ SÓ
--
-- Cole tudo no SQL Editor de PRODUÇÃO e clique em Run. Uma vez.
--
-- Este arquivo junta TUDO o que falta em produção:
--   - as 5 migrations da #49 (rescisão separada da devolução do caução);
--   - a coluna do horário do recebimento nas parcelas de caução.
--
-- Ele SUBSTITUI o PROD-rescisao-49.sql. Se você já rodou aquele, pode rodar
-- este mesmo assim: tudo aqui é idempotente (rodar de novo não estraga nada).
--
-- É SEGURO:
--   - pode rodar de novo sem estragar nada;
--   - só acrescenta colunas e ajustes; não apaga nem reescreve dado de
--     ninguém, com uma exceção descrita no passo 4 (parcelas de caução
--     gravadas como "pagas por R$ 0,00", que são corrigidas);
--   - no fim imprime um relatório: as 6 linhas devem dizer OK.
--
-- ORDEM DO DIA:
--   1. rode este arquivo em PRODUÇÃO;
--   2. confira as 6 linhas do relatório;
--   3. só então publique o código novo.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 6. Horário do recebimento nas parcelas de caução
--
-- A tela "Registrar Recebimento de Caução" ganhou o campo "Horário do
-- Recebimento", que as outras três telas já tinham. A coluna nunca existiu
-- nesta tabela. Registros antigos ficam vazios.
--
-- Seguro: só acrescenta coluna.
-- ----------------------------------------------------------------------------
...
SELECT '5. card do Kanban',
       CASE WHEN EXISTS (SELECT 1 FROM kanban_cards
            ...
            THEN 'OK' ELSE 'FALTOU' END
UNION ALL
SELECT '6. horario nas parcelas de caucao',
