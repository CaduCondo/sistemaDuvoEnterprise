...
export default function KanbanPage() {
  ...
  const moveColumn = (card: KanbanCard, direction: 1 | -1) => {
    ...
    moveCard(card.id, COLUMNS[nextIndex].key);
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Backlog de bugs, features e melhorias do sistema. Qualquer admin ou corretor pode incluir e atualizar itens.
            </p>
          </div>
            ...
            {COLUMNS.map((column, columnIndex) => (
              <div key={column.key} className="space-y-3">
                <div className="flex items-center justify-between px-1">
                  ...
                  <Badge variant="secondary">{cardsByColumn[column.key].length}</Badge>
                </div>

                <div className="space-y-3 min-h-[100px]">
                  {cardsByColumn[column.key].length === 0 ? (
                    <p className="text-xs text-muted-foreground px-1">Nenhum item aqui.</p>
                  ) : (
                    cardsByColumn[column.key].map((card) => (
                      <Card
                        key={card.id}
